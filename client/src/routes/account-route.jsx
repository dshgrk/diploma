            ...
            <div className="container account-dashboard-grid">
              <section className="account-profile-card">
                ...
                <div className="account-profile-meta">
                  <div>
                    <span>{locale === "uk" ? "Email" : "Email"}</span>
                ...
                {!currentOrder ? (
                  ...
                  <div className="account-order-summary">
                    <div className="account-order-meta-row">
                      <div>
                        <span>{locale === "uk" ? "Номер" : "Order"}</span>
                        <strong>{currentOrder.order_number}</strong>
                      </div>
                      <div>
                        <span>{locale === "uk" ? "Створено" : "Created"}</span>
                        <strong>{formatOrderDate(currentOrder.created_at, localeFormat)}</strong>
                      </div>
                      <div>
                        <span>{locale === "uk" ? "Оновлено" : "Updated"}</span>
                        <strong>{formatOrderDate(currentOrder.updated_at, localeFormat)}</strong>
                      </div>
                      <div>
                        <span>{t(locale, "orderTotal")}</span>
                        <strong>{formatCurrency(currentOrder.total_amount, currentOrder.currency, localeFormat)}</strong>
                      </div>
                    </div>
                    <div className="account-order-items">
                      {currentOrder.items.map((item) => (
                        <AccountOrderLine key={item.id} item={item} order={currentOrder} locale={locale} localeFormat={localeFormat} />
                      ))}
                    </div>
