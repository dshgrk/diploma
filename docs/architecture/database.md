...
- Основная СУБД в актуальном Docker-first runtime: `SQLite` через `sqlite3` и Knex.
...
AND datetime(in_progress_at, '+14 days') < CURRENT_TIMESTAMP
```

В текущей реализации значение вычисляется на уровне сервисного слоя, чтобы не хранить производное состояние в базе и не привязывать бизнес-логику к конкретному SQL-диалекту.
